...
function falbum_options_page() {
	...
	$rewriteRule = "<IfModule mod_rewrite.c>\n"."RewriteEngine On\n"."RewriteRule ^".$furl."?([^/]*)?/?([^/]*)?/?([^/]*)?/?([^/]*)?/?([^/]*)?/?([^/]*)?/?([^/]*)?/?([^/]*)?/?$ ".$path."/wp-content/themes/".get_bloginfo('template')."/plugins/falbum/wp/album.php?$1=$2&$3=$4&$5=$6&$7=$8 [QSA,L]\n"."</IfModule>";
...
?> 		<div class="updated"><p><strong><?php echo $updateMessage?></strong></p></div> <?php
	...
	if (!isset ($options['url_root']) || $options['url_root'] == "") {
		$options['url_root'] = $path."/wp-content/themes/".get_bloginfo('template')."/plugins/falbum/wp/album.php";
	}
	...
	if (!isset ($options['url_falbum_dir']) || $options['style'] == "") {
		$options['url_falbum_dir'] = $path."/wp-content/themes/".get_bloginfo('template')."/plugins/falbum";
	}
...
?>
	...
	echo $path."/wp-content/themes/".get_bloginfo('template')."/plugins/falbum/wp/album.php";
?>
				   </td>
                </tr>
	...
	echo $path."/wp-content/themes/".get_bloginfo('template')."/plugins/falbum";
?>
				   </td>
                </tr>
	...
	if ((defined('FALBUM') && constant('FALBUM')) || $falbum_options['wp_enable_falbum_globally'] == 'true') {
		...
		if ($style == '_current_wordpress_theme') {
			...
			if (file_exists($tdir_uri."/falbum/falbum.css")) {
				...
				$cssUrl = get_settings('siteurl')."/wp-content/themes/".get_bloginfo('template')."/plugins/falbum/styles/default/falbum.css";
			}		  
	      	...
	      	if (file_exists($tdir."/falbum.css")) {
	  			...
	  			$cssUrl = get_settings('siteurl')."/wp-content/themes/".get_bloginfo('template')."/plugins/falbum/styles/".$style."/falbum.css";
	  		}
